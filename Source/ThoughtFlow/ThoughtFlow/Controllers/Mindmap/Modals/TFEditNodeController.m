...
@implementation TFEditNodeController
...
- (void) _notifyEditNodeControllerDismissedWithName: (NSString *) name {
    if (delegate && [delegate respondsToSelector: @selector(editNodeController:dismissedWithName:)]) {
        [delegate editNodeController: self dismissedWithName: name];
    }
}
@end